// v3
import { findByProps, findByStoreName, findByTypeNameAll } from "@revenge-mod/modules/metro";
...
const rowPatch = ([{ guildId, user }], res) => {
    ...
    const nameContainer = findInReactTree(label, (c) =>
        ...
        c.props.children.some((ch) => typeof ch === "string" || typeof ch?.props?.children === "string")
    );
    ...
    if (tag) {
        if (existingTag) {
            Object.assign(existingTag.props, { type: 0, text: tag.text, textColor: tag.textColor, backgroundColor: tag.backgroundColor, verified: tag.verified });
        } else {
            if (!Array.isArray(nameContainer.props.children)) nameContainer.props.children = [nameContainer.props.children];
            nameContainer.props.children.push(
                <TagModule.default type={0} text={tag.text} textColor={tag.textColor} backgroundColor={tag.backgroundColor} verified={tag.verified} />
            );
        }
    }
};
